...
class UVSheetParser:
    ...
    def run(self, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            for idx, row in enumerate(rows, 1):
                ...
                success = self._process_row(row, dry_run)
    ...
    def _process_row(self, row: dict, dry_run: bool = False) -> bool:
        """
        ...
        try:
            ...
            # Ensure num_sheets is int
            try:
                num_sheets = int(num_sheets)
            except (ValueError, TypeError):
                logger.error(f"Invalid 'Number of Sheets' value: {num_sheets}")
                return False
            ...
            if dry_run:
                logger.info("[DRY RUN] Would create experimental step and child samples")
            ...
            if len(children) < num_sheets:
                logger.warning(f"Only created {len(children)}/{num_sheets} child samples")
